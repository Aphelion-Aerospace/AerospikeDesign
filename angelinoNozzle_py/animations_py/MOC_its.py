...
class chr_point():
    ...
    def plot_point(self):
        self.axis.plot(self.x,self.y,'bx')
        self.axis.plot(self.x,self.y*-1,'bx')

        # will calculate other properties later (p etc.) to vectorize
class chr_mesh():
    def __init__(self,spike,gamma,altitude,n,num_its,downstream_factor=1.1,plot_chr=0,clean_mesh=1,axis=plt):

        self.spike =copy.copy(spike); self.gamma = gamma; self.altitude =altitude; self.n = n; self.num_its = num_its; self.axis=axis
        ...
        #     self.axis.plot(point.x,point.y,'rX')       
        ...
        ## TODO: COMPUTE EXPANSION FAN UNTIL POINT IS > spike.length in which case, remove from all tracking lists and do not add to chr_array
            ...
            if(self.new_fan):
                ...
                # self.axis.plot(self.chr_array[self.ID_jet_boundary[-1]].x,self.chr_array[self.ID_jet_boundary[-1]].y,'gx')
                ...
                self.chr_array = np.append(self.chr_array,new_point)
                #self.axis.plot(new_point.x,new_point.y,'gx')
                ...
                if(self.chr_array[ID_temp].pt_type!="same_fam"):
                    ...
                    if (temp1.x < temp2.x) and (temp1.x>self.chr_array[ID_temp].x) :
                        ...
                        #print(temp1.x)
                        if (self.plot_chr):
                            self.axis.plot(self.chr_array[self.ID_right_chr[0]].x,self.chr_array[self.ID_right_chr[0]].y,'bx',self.chr_array[ID_temp].x,self.chr_array[ID_temp].y,'rx')
                            self.axis.plot(self.chr_array[self.ID_right_chr[0]].x,self.chr_array[self.ID_right_chr[0]].y*-1,'bx',self.chr_array[ID_temp].x,self.chr_array[ID_temp].y*-1,'rx')
                            self.axis.plot(temp1.x,temp1.y,'go')
                            self.axis.plot(temp1.x,temp1.y*-1,'go')
                            #self.axis.plot([self.chr_array[ID_temp].x, temp1.x],[self.chr_array[ID_temp].y, temp1.y])
                        ...
                        #self.axis.plot(self.chr_array[self.ID-1].x,self.chr_array[self.ID-1].y,'ro')
                        self.ID += 1      
                    else:
                        new_point = temp2
                        if (new_point.x<=self.spike.length):
                            pass
                        if (self.plot_chr):
                            self.axis.plot([self.chr_array[ID_temp].x, temp2.x],[self.chr_array[ID_temp].y*-1, temp2.y*-1],'k',[self.chr_array[self.ID-1].x, temp2.x],[self.chr_array[self.ID-1].y*-1, temp2.y*-1],'k')  
                            self.axis.plot([self.chr_array[ID_temp].x, temp2.x],[self.chr_array[ID_temp].y, temp2.y],'k',[self.chr_array[self.ID-1].x, temp2.x],[self.chr_array[self.ID-1].y, temp2.y],'k')                    
    ...
    def general_point(self,A,B,plot_chr=0):
        ...
        if (x_c <= self.spike.length):
            ...
            # self.ID_right_chr.append(self.ID)              
        if (plot_chr):
            self.axis.plot([A.x,x_c],[A.y,y_c],'k',[B.x,x_c],[B.y,y_c],'k')
            self.axis.plot([A.x,x_c],[A.y*-1,y_c*-1],'k',[B.x,x_c],[B.y*-1,y_c*-1],'k')
    ...
    def same_fam_point(self,A,B,plot_chr=0):
        ...
        with np.errstate(invalid='ignore',divide='ignore'):
            ...
            W_c = A.W + A.W*(np.tan(A.mu*(theta_c-A.theta))+l_A/A.y*(x_c-A.x))
        # if (self.plot_chr):
        #     self.axis.plot([A.x,x_c],[A.y,y_c],'r',[B.x,x_c],[B.y,y_c],'r')
        if (x_c <= self.spike.length):
            pass
        if (plot_chr):
            self.axis.plot([A.x,x_c],[A.y,y_c],'k',[B.x,x_c],[B.y,y_c],'k')
    ...
    def contour_point(self,A,plot_chr=0):
        ...
        #self.axis.plot(x_c,y_c,'rX')
        # self.ID_contour_chr.append(self.ID)
        self.ID_contour_chr.append(self.ID)  
        if (x_c <= self.spike.length):  
            pass  
        if(plot_chr):
            self.axis.plot([A.x,x_c],[A.y,y_c],'k')
            self.axis.plot([A.x,x_c],[A.y*-1,y_c*-1],'k')
    ...
    def jet_boundary_point(self,A,B,plot_chr=0):
        ...
        if (plot_chr):
            self.axis.plot([A.x,x_c],[A.y,y_c],'r',[B.x,x_c],[B.y,y_c],'r')
            self.axis.plot([A.x,x_c],[A.y*-1,y_c*-1],'r',[B.x,x_c],[B.y*-1,y_c*-1],'r')
    ...
    def internal_jet_point(self,A,B,plot_chr=0):
        ...
        if (x_c <= 2*self.spike.length):
            pass
        if (plot_chr):
            self.axis.plot([A.x,x_c],[A.y,y_c],'g',[B.x,x_c],[B.y,y_c],'g')
            self.axis.plot([A.x,x_c],[A.y*-1,y_c*-1],'g',[B.x,x_c],[B.y*-1,y_c*-1],'g')
    ...
    def compute_thrust(self,approx_method,n):       
        ...
        #self.axis.plot(x_plane,y_points,'ro')
        ...
        # print(vel_label)
        # self.axis.plot(y_points,Ve_grid,label = vel_label)
        ...
        # print('thurst!')
        # self.axis.plot(on_contour_x,on_contour_y,'x')
        # plt.show()
        ...
        # self.axis.plot(y_points,np.cos(theta_grid),label =p_label)
        # plt.legend()
        #plt.show()
        A = y_points*2*np.pi
...
# #self.axis.plot(MOC_mesh.x,MOC_mesh.y,'ro')

# #self.axis.plot(MOC_mesh.x[MOC_mesh.ID_jet_boundary],MOC_mesh.y[MOC_mesh.ID_jet_boundary],'go-')
# MOC_mesh.compute_thrust('nearest',10)
# #self.axis.plot(MOC_mesh.x[MOC_mesh.ID_contour_chr],MOC_mesh.y[MOC_mesh.ID_contour_chr],'bo-')

# #self.axis.plot(MOC_mesh.x[MOC_mesh.ID_contour_chr[-1]],0,'bo')
# #self.axis.plot(spike.x,interpolate.splev(spike.x,tck,der=0),spike.lip_x,spike.lip_y,'X')
# #plt.axis('equal')
# # fig = plt.figure()
# # ax = fig.add_subplot(111)
# # ax.scatter(MOC_mesh.x,MOC_mesh.y,c=MOC_mesh.M, cmap = cm.coolwarm)
# # #self.axis.plot(MOC_mesh.x,MOC_mesh.y,'r.')
