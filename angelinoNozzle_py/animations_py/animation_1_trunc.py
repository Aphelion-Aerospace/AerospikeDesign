...
class aerospike_optimizer():
	...
	def animate_to_its(self,num_its):
		...
		for its in num_its:
			...
			spike.plot_contour(ax1)
			spike.y = spike.y*-1; spike.lip_y = spike.lip_y*-1
			spike.plot_contour(ax1)
			ax1.plot([spike.x[-1],spike.x[-1]],[spike.y[-1]*-1,spike.y[-1]],'b-')

			try:
				MOC_mesh = MOC_its.chr_mesh(spike,self.CEA.gamma,self.design_alt_init,30,its,downstream_factor=1.1,plot_chr=1,axis=ax1)
			...
			name = 'animation_1_trunc/fig' + str(its)
			plt.title('Mesh Generation Full Length',fontsize=40)
	...
	def animate_over_range(self,plug_nozzle_class,alt_range,downstream_factor=1.2,chr_mesh_n=50):
		...
		for i in range(alt_range.shape[0]):
			...
			plt.ylim((-0.1,0.1))
			plt.xlim((-0.01,0.26))
			name = 'animation_2_full_length/fig' + str(alt_range[i])
	...
	def multicore_animate_range(self,gamma,downstream_factor=1.2,chr_mesh_n=50,no_core=1):
 #3.28084

		alt_range = np.linspace(0,12000,self.no_alt_range_init)
		alt_range_split = np.split(altitude_range,no_core)
...
if __name__ == '__main__':
	...
	#plt.legend()
	optimizer.multicore_animate_to_its(680,4)
