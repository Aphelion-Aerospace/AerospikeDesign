...
class aerospike_optimizer():
	...
	def animate_over_range(self,plug_nozzle_class,alt_range,flight_range,downstream_factor=1.2,chr_mesh_n=50):
		...
		for i in range(alt_range.shape[0]):
			...
			M_fill = ax1.contourf(X_plt,Y_plt,M_contour)
			ax1.contourf(X_plt,-1*Y_plt,M_contour)
			plt.colorbar(M_fill,ax=ax1)
			name = 'animation_3_full_length/fig' + str(int(alt_range[i]))
			...
			ax2.plot(x_flight,self.__fake_flight_parabola(x_flight))
			ax2.plot(flight_range[i],self.__fake_flight_parabola(flight_range[i]),'rx')
			ax2.set_xlim([0,1])
			ax2.set_ylim([0,1])
...
if __name__ == '__main__':
	...
	optimizer = aerospike_optimizer(r_e,T_w,alpha,beta,design_alt,truncate_ratio,chr_mesh_n=30,no_alt_range = 300,no_core=4)

	optimizer.multicore_animate_range(downstream_factor=1.2,chr_mesh_n=100,no_core=4)
