...
class aerospike_optimizer():
	...
	def animate_over_range(self,plug_nozzle_class,alt_range,downstream_factor=1.2,chr_mesh_n=50):
		...
		for i in range(alt_range.shape[0]):
			...
			plug_nozzle_class.plot_contour(plt)
			MOC_mesh = MOC.chr_mesh(plug_nozzle_class,CEA.gamma,alt_range[i],chr_mesh_n,downstream_factor=downstream_factor,plot_chr=1,clean_mesh=0)
...
if __name__ == '__main__':
	...
	optimizer = aerospike_optimizer(r_e,T_w,alpha,beta,design_alt,truncate_ratio,chr_mesh_n=30,no_alt_range = 800,no_core=4)
